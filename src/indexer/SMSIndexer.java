...
public class SMSIndexer {
    ...
    public SMSIndexer(String propFile) throws Exception {
        prop = new Properties();
    ...
    void processAll() throws Exception {
        System.out.println("Indexing SMS collection...");
...
//    
    ...
    Document constructDoc(String[] tweetCSV) throws Exception {
        Document doc = new Document();
        String docTextElt = "";
        for (int i = 0; i < tweetCSV.length - 1; i++) {
            docTextElt += tweetCSV[i];
        }
        String docDomainName = tweetCSV[tweetCSV.length - 1];
//        String docTimeStringElt = tweetCSV[1];
//        String docTimeElt = tweetCSV[2];
//        String docUserElt = tweetCSV[4];
        doc.add(new Field(WMTIndexer.FIELD_DOMAIN_ID, docDomainName, Field.Store.YES, Field.Index.NOT_ANALYZED));
...
//        doc.add(new Field(TweetFields.FIELD_USERNAME, docUserElt, Field.Store.YES, Field.Index.NOT_ANALYZED));
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            ...
            args[0] = "sms.properties";
        }

        try {
            SMSIndexer indexer = new SMSIndexer("sms.properties");
            indexer.processAll();
        }
        catch (Exception ex) {
            ex.printStackTrace();
